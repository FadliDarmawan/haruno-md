...
handler.before = async function (m) {
    ...
    if (!(id in this.siapakahaku)) return this.sendButton(m.chat, 'Soal itu telah berakhir', watermark, ['siapakahaku', '/siapakahaku'], m)
    if (m.quoted.id == this.siapakahaku[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            this.sendButton(m.chat, `*Benar!*\n+${this.siapakahaku[id][2]} XP`, watermark, ['siapakahaku', '/siapakahaku'], m)
